...
const Blog = ({ id, image, heading, date, paragraph, tag, details }) => {
  return (
    ...
    >
      {/* Outer container */}
      <div className="inline-block h-full bg-bg-muted p-4 md:p-8 lg:p-12">
        {/* Entire blog card is wrapped in a Link */}
        <Link href={details} className="block">
          ...
          <div className="mb-16 mt-8">
            ...
            <h4 className="font-satoshi mb-2 text-h4">{heading}</h4>
            <p className="font-chillax text text-text-muted">{paragraph}</p>
          </div>
        </Link>

        {/* Tag Link placed outside the main Link but absolutely positioned */}
        <Link
          href={`/tags/${tag}`}
          className="absolute bottom-12 left-12 border border-border bg-bg-muted p-2 text-meta-sm uppercase text-text"
        >
          {tag}
        </Link>
      </div>
